...
public class ModelRuntimeController {
    ...
    public ApiResponse<ModelRuntime> model() {
        var route = modelRouter.resolve(LogicalModelType.RESEARCH_FAST);
        return ApiResponse.ok(new ModelRuntime(
                route.model().provider(), mode, route.model().modelName(), externalEnabled));
    }
