...
| 质量属性 | 设计响应 |
| --- | --- |
| 隐私 | 默认 Mock；外部模型双开关；最终出站门禁；高风险内容禁止外发 |
...
当前 Windows 脚本使用此拓扑，并在本地 HTTP 下显式覆盖 Secure Cookie 以便开发。
...
当前仓库没有生产级 Helm、Terraform、镜像或反向代理配置。部署前还需要补齐备份恢复、密钥轮换、
监控告警、网络策略、容量、容灾和变更回滚方案。
...
| 前端 | Vitest、Playwright 多轮澄清和检索式确认 |
...
- 缺少生产级健康端点、指标、分布式追踪和集中日志约定。
- 缺少正式备份恢复、密钥轮换和灾难恢复文档。
